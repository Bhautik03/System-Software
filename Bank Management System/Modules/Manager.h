...
label1:
    ...
    int login_result = loginManager(connectionFD, mngID, password);
    if(login_result == 1)
    {
        bzero(writeBuffer, sizeof(writeBuffer));
        bzero(readBuffer, sizeof(readBuffer));
...
void assignLoanApplication(int connectionFD)
{
    ...
    // Check if there are any unassigned loans
    int hasUnassignedLoan = 0;
    ...
    // If no unassigned loans, show message and return
    if(!hasUnassignedLoan)
    {
        close(file);
        bzero(readBuffer, sizeof(readBuffer));
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer, "No loans available to assign^");
        write(connectionFD, writeBuffer, sizeof(writeBuffer));
        read(connectionFD, readBuffer, sizeof(readBuffer));
        return;
    }
