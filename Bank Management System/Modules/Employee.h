...
label1:
    ...
    int login_result = loginEmployee(connectionFD, empID, password);
    if(login_result == 1)
    {
        bzero(writeBuffer, sizeof(writeBuffer));
        bzero(readBuffer, sizeof(readBuffer));
            ...
            switch(choice)
            {
                ...
                case 6:
                    ...
                    else
                    {
                        ...
                        write(connectionFD, writeBuffer, sizeof(writeBuffer));
                        read(connectionFD, readBuffer, sizeof(readBuffer));
                        // Release semaphore before re-login
    ...
    else
    {
        ...
        bzero(readBuffer, sizeof(readBuffer));
        read(connectionFD, readBuffer, sizeof(readBuffer));
        int retryChoice = atoi(readBuffer);
        if(retryChoice == 1) goto label1; else return;
    }
}
...
void approveRejectLoan(int connectionFD, int empID)
{
    ...
    // Check if there are any pending loans assigned to this employee
    int hasAssignedLoan = 0;
    while(read(file, &ld, sizeof(ld)) != 0)
    {
        if(ld.empID == empID && ld.status == 1)  // status 1 = pending
        {
            hasAssignedLoan = 1;
            break;
        }
    ...
    // If no pending loans assigned, show message and return
    if(!hasAssignedLoan)
    {
        close(file);
        close(fp);
        bzero(readBuffer, sizeof(readBuffer));
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer, "No loans assigned to you for approval/rejection^");
        write(connectionFD, writeBuffer, sizeof(writeBuffer));
        read(connectionFD, readBuffer, sizeof(readBuffer));
        return;
    }
    ...
    // Display all pending loans assigned to this employee
    while(read(file, &ld, sizeof(ld)) != 0)
    {
        if(ld.empID == empID && ld.status == 1)  // status 1 = pending
        {
    ...
    int isValidLoan = 0;  // Check if loan is assigned to this employee and is pending
    struct LoanDetails foundLoan;  // Store found loan for validation
    
    while (read(file, &ld, sizeof(ld)) != 0)
    {
        if(ld.loanID == lID)
        {
            // Store the found loan details
            foundLoan = ld;
            sourceFound1 = 1;
            
            // Check if loan is assigned to this employee and is pending
    ...
    // If loan ID not found or not assigned to this employee or not pending
    if(!sourceFound1 || !isValidLoan)
    {
        close(file);
        close(fp);
        bzero(writeBuffer, sizeof(writeBuffer));
        ...
        else if(foundLoan.empID != empID)
        {
            sprintf(writeBuffer, "Loan ID %d is not assigned to you^", lID);
        }
        else if(foundLoan.status != 1)
        {
            sprintf(writeBuffer, "Loan ID %d is already either approved or rejected^", lID);
        }
        else
        {
    ...
    // Use the validated loan details
    ld = foundLoan;
    ...
    {
        // Could not acquire lock - loan might be in use
        close(fp);
        bzero(writeBuffer, sizeof(writeBuffer));
        bzero(readBuffer, sizeof(readBuffer));
        sprintf(writeBuffer, "Unable to process loan ID %d. Please try again later^", lID);
        write(connectionFD, writeBuffer, sizeof(writeBuffer));
        read(connectionFD, readBuffer, sizeof(readBuffer));
    }
