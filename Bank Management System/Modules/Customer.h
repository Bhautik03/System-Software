...
label1:
    ...
    else
    {
        ...
        int login_result = loginCustomer(connectionFD, accountNumber, password);
        if (login_result == 1)
        {            
            while(1)
            {   
...
int loginCustomer(int connectionFD, int accountNumber, char *password) {
    ...
    if (sem_trywait(sema) == -1) {
        ...
        close(file);
        // Return a special code for already logged in
    ...
    while(read(file, &customer, sizeof(customer)) != 0)
    {
        if (customer.accountNumber == accountNumber) {
            if (strcmp(customer.password, crypt(password, HASHKEY)) == 0) {
                if (customer.activeStatus == 1) {
                    printf("Customer whose acc no.: %d loggedIn\n", accountNumber);
                    close(file);
                    return 1;
                } else {
                    // Release semaphore before returning for inactive account
...
void transferFunds(int connectionFD, int sourceAccount, int destAccount, float amount) {
    ...
    sprintf(transactionBuffer,"%.2f transferred into acc no %d at %02d:%02d:%02d %d-%d-%d\n", amount, destAccount, current_time->tm_hour,current_time->tm_min,current_time->tm_sec,(current_time->tm_year)+1900,(current_time->tm_mon)+1,current_time->tm_mday);
    bzero(th.hist, sizeof(th.hist));
    strcpy(th.hist, transactionBuffer);
    ...
    bzero(transactionBuffer, sizeof(transactionBuffer));
    sprintf(transactionBuffer,"%.2f credited by acc no %d at %02d:%02d:%02d %d-%d-%d\n", amount, sourceAccount, current_time->tm_hour,current_time->tm_min,current_time->tm_sec,(current_time->tm_year)+1900,(current_time->tm_mon)+1,current_time->tm_mday);
    bzero(th.hist, sizeof(th.hist));
    strcpy(th.hist, transactionBuffer);
